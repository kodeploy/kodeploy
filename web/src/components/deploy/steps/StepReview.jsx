...
import { Disclosure, StepHeading } from "./wizardParts.jsx";
...
export default function StepReview({
  ...
  return (
    <div style={{ paddingTop: 56 }}>
      ...
      {/* 환경변수 — 값이 있으면 이름만 펼쳐 보여준다(값은 배포 전 확인 화면에서 노출하지 않는다). */}
      <div style={{ minHeight: ROW_H, display: "flex", alignItems: "center", borderBottom: "1px solid var(--kd-border)" }}>
        <div style={{ width: "100%" }}>
          <Disclosure
            open={showEnv}
            onToggle={onToggleEnv}
            label="환경변수"
            caption={envCount > 0 ? `${envCount}개` : "추가된 값 없음"}
          >
            <ul className="kd-t-code" style={{ color: "var(--fg-2)", listStyle: "none" }}>
              {envSummary.length > 0 ? (
                envSummary.map((k) => <li key={k}>{k}</li>)
              ) : (
                <li style={{ color: "var(--fg-4)" }}>없음</li>
              )}
            </ul>
          </Disclosure>
        </div>
